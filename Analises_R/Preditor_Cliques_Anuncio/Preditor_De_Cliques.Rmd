...
![](_99764882_gettyimages-591853022.jpg)
...
# Verificando se há dados faltantes nas variáveis

```{r}
df %>% is.na %>% colSums %>% knitr::kable(align = "l")
```
...
plot_ly(data = predicoes, y=~Predicao, x=~N, color=~`Clicou no anuncio`, type = "scatter", colors = c("firebrick","olivedrab"))
```
...
predicoes[,-1] %>% head %>% knitr::kable(align = "ll")
```
